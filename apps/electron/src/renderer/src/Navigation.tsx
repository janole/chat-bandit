...
import { FlexBox, QuickMenu, SplitButton, TagButton } from "@renderer/BasicApp";
import { useChatStore, useChatClient, useNewChat } from "@libraries/ai/ElectronClient";
import { create } from "zustand";
import { CancelButton } from "./BasicApp/Components/SpinningButton";
import { Spacer } from "./BasicApp/Components/FlexBox";
import { Copy } from "lucide-react";
import Mark from "mark.js";
import { IChat } from "@libraries/ai/types";
...
function ChatListItem(props: { id: string })
{
    ...
    const { abortChat, showChatFileInFileManager } = useChatClient();
    const deleteChat = useChatStore(state => state.deleteChat);
    const undeleteChat = useChatStore(state => state.undeleteChat);
    const { branchChat } = useNewChat();
    ...
    return (
        ...
        >
            ...
            <FlexBox gap={0.5}>
                ...
                // onMenuVisibilityChange={visible => setHover(visible)}
                />
            </FlexBox>
        </ListItemButton>
    );
